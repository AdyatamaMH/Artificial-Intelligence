...
inputs = tokenizer(
    list(df['content']),  # Use the 'content' column
    return_tensors="pt",
    padding=True,
    truncation=True,
    max_length=512
...
labels = torch.tensor(df['status'].tolist(), dtype=torch.long)  # Assuming 'status' is the target label column
...
model_save_path = "indobert_trained_model.pth"
